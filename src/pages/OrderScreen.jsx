...
export default function OrderScreen() {
  ...
  const handleCategoryClick = (catId) => {
    ...
    if (catObj && catObj.subCategories && catObj.subCategories.length > 0) setActiveSubCat(catObj.subCategories[0].id);
    else setActiveSubCat(null);
  };

  // YENİ: VERİTABANINDAKİ SIRALAMAYA GÖRE DİZİLİR
  const items = useMemo(() => {
    if (!menu) return [];
  ...
  const handleSend = async () => {
    ...
    // OPTIMISTIC UI: HIZ SORUNU İÇİN ANINDA KAPAT VE TEMİZLE
    setSending(true);
    ...
    try {
      ...
      // Her şeyi temizle (CartBar'ın da kapanmasını sağlamak için)
      setCart([]);
      setTableNumber("");
      setOrderNote(""); 
      ...
      <div className="flex h-full flex-col">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin">
          <div className="mx-auto max-w-7xl px-4 py-5 pb-32">
            ...
            {/* Kategori Başlığı ve ÖZEL ÜRÜN Butonu */}
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-muted-foreground">
                {activeSubCat ? CATEGORIES.find(c => c.id === activeCat)?.subCategories.find(s => s.id === activeSubCat)?.label : CATEGORIES.find(c => c.id === activeCat)?.label}
              </h2>
              <Button onClick={() => setIsCustomOpen(true)} variant="outline" size="sm" className="border-dashed border-2 border-primary text-primary hover:bg-primary/10">
                <PenLine className="w-4 h-4 mr-2" /> Özel Ürün Ekle
              </Button>
            </div>
